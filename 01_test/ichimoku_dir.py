...
for i in range(len(data_list)):
    ...
    data_dir = 'Stocks/'+str(data_list[i])
    data = ichimoku(data_dir)
    data = np.array(data).transpose()
    try:
        if data[3].max() <= 50:
            shutil.copy(data_dir, 'max_50')
        elif data[3].max() <= 100:
            shutil.copy(data_dir, 'max_100')
        elif data[3].max() <= 150:
            shutil.copy(data_dir, 'max_150')
        elif data[3].max() <= 200:
            shutil.copy(data_dir, 'max_200')
        else:
            shutil.copy(data_dir, 'max_1000')
        print('done')
    except:
        print('-----------------------------------')
        print(data_dir)
